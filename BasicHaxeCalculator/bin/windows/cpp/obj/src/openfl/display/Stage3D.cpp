		...
		if ((tmp)){
			...
			Void run(){
				HX_STACK_FRAME("*","_Function_2_1",0x5201af78,"*._Function_2_1","openfl/display/Stage3D.hx",31,0x37141b9b)
				{
					...
					_g->context3D = tmp1;
					HX_STACK_LINE(34)
					::openfl::events::Event tmp2 = ::openfl::events::Event_obj::__new(HX_HCSTRING("context3DCreate","\x7c","\xbf","\x59","\x7b"),null(),null());		HX_STACK_VAR(tmp2,"tmp2");
					HX_STACK_LINE(34)
			...
			Void run(){
				...
				{
					HX_STACK_LINE(42)
					::openfl::events::ErrorEvent tmp1 = ::openfl::events::ErrorEvent_obj::__new(HX_HCSTRING("error","\xc8","\xcb","\x29","\x73"),null(),null(),null(),null());		HX_STACK_VAR(tmp1,"tmp1");
					HX_STACK_LINE(42)
					_g->dispatchEvent(tmp1);
				}
				return null();
			}
			HX_END_LOCAL_FUNC0((void))
